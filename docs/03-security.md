...
- 対象: Azure 上の全コンポーネント（リソース設計書 #44 記載の12リソース）
- PoC 構成（10名）の設計を記述
...
シークレット一覧および RBAC 設定の詳細はリソース設計書（#44）を参照。
...
| 文書 | 内容 |
|------|------|
| 要件定義書（#42） | セキュリティ要件（NF-S01〜S05） |
| アーキテクチャ設計書（#43） | 認証経路・コンポーネント構成 |
| リソース設計書（#44） | RBAC 設定・Key Vault シークレット一覧 |
