...
| 0.3 | 2026-03-20 | 構築担当者 | 変更検知・増分インデックス、FORCE_REINDEX、ACL 自動同期セクション追加。コスト情報を Basic SKU に更新 |
...
| ~~Document Intelligence~~ | ~~`di-sprag-poc-jpe`~~ | — | **削除済み**。DI Layout は Cognitive Services マルチサービスアカウント経由で利用 |
...
4. ~~Document Intelligence (`di-sprag-poc-jpe`)~~ — **削除済み**（DI Layout は Cognitive Services マルチサービスアカウント経由）
5. Storage Account (`stspragpocjpe`) + コンテナ `sharepoint-documents`
6. AI Search (`srch-sprag-poc-jpe`) — **Basic 課金リソース 約 $75/月**
...
データソース (`datasource.json`) に変更検知ポリシーが設定されており、インデクサーは変更/新規ドキュメントのみを処理する。

| ポリシー | 設定 | 動作 |
|---------|------|------|
| 変更検知 | `HighWaterMarkChangeDetectionPolicy` (`metadata_storage_last_modified`) | Blob の最終更新日時を比較し、変更があったドキュメントのみ DI Layout で再処理 |
| 削除検知 | `NativeBlobSoftDeleteDeletionDetectionPolicy` | Blob の論理削除を検知し、インデックスから対応ドキュメントを自動削除 |
| スケジュール | `PT24H`（24時間ごと） | インデクサーが日次で自動実行 |

定常運用では変更がないドキュメントは DI Layout を通らないため、DI の従量課金は実質 $0 になる。

### 4.4 rebuild.sh と FORCE_REINDEX

`scripts/rebuild.sh` はインフラ再構築 + Search オブジェクトデプロイ + インデクサー実行 + ACL 同期を一括で行うスクリプト。

| モード | コマンド | 動作 |
|--------|---------|------|
| **増分（デフォルト）** | `bash scripts/rebuild.sh` | インデックス削除をスキップ、インデクサーリセットをスキップ。変更検知により変更分のみ再処理 |
| **フルリビルド** | `FORCE_REINDEX=true bash scripts/rebuild.sh` | インデックスを削除して再作成、インデクサーをリセットして全ドキュメントを DI Layout で再処理 |

フルリビルドが必要なケース:
- スキルセットの設定変更（チャンクサイズ変更等）
- インデックスのスキーマ変更（フィールド追加等）
- シノニムマップの大幅変更
- インデクサーが transientFailure を繰り返す場合

> rebuild.sh は完了時に自動で `update_index_metadata.py` を実行するため、手動での ACL 同期は不要。

### 4.5 ACL メタデータ自動同期

`rebuild.sh` はインデクサー完了待ち後に `update_index_metadata.py` を自動実行する（ステップ 10/13）。これにより `allowed_groups`, `category`, `source_url` が全チャンクに反映される。

**権限のみの変更**（ドキュメント内容は変更なし、SP フォルダ権限のみ変更の場合）:

```bash
cd scripts
source .venv/Scripts/activate

# 1. SP → Blob 同期（権限メタデータを更新）
python sp_to_blob.py

# 2. インデックスの ACL メタデータを更新
python update_index_metadata.py
```

> この場合インデクサーの再実行は不要。Blob メタデータの更新 + インデックスへの直接 merge で完了する。

### 4.6 CI/CD によるコードデプロイ
...
| 優先度 | リソース | 月額 | 操作 |
|--------|---------|------|------|
| 1 | AI Search Basic | ~$75 | 削除（停止不可） |
| 2 | App Service B1 | ~$13 | App Service プラン削除 |
| 3 | その他 | ~$2 | 従量課金のため使用しなければほぼ $0 |
